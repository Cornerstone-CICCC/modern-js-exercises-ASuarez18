...
administration has been making to the local infrastructure, and they want more! The parking lot 
in the Codeville Devtropolis Shopping Mall needs an upgrade, and you've decided this is the perfect 
opportunity to install a smart parking system.

The system will use special parking sensors to keep track of all parking spots and monitor 
which ones are available. Every time a vehicle enters the parking lot, the system directs them to 
an available spot for their particular vehicle type, or notifies them that no spots are available.

? Instruction
We need to write a function called whereCanIPark() that returns the coordinates of an available 
parking spot for the vehicle, or returns false if there is no available spot. Our function receives an
array of arrays representing parking spots, and a string with type of the vehicle that is looking for a parking spot.
...
const whereCanIPark = function (spots, vehicle) {
  // Code here!
  let size = vehicle === "regular" ? ['R'] : vehicle === "small" ? ['R', 'S'] : ['R', 'S', '']
  // TODO for
  for (const i in spots) {
    for (const j in spots) {
      switch (vehicle) {
        case "regular":
          if (spots[i][j] === 'R') return [parseInt(j), parseInt(i)];
          break;
        case "small":
          if (spots[i][j] === 'R' || spots[i][j] === 'S') return [parseInt(j), parseInt(i)];
          break;
        case "motorcycle":
          if (spots[i][j] === 'R' || spots[i][j] === 'S' || spots[i][j] === 'M') return [parseInt(j), parseInt(i)];
          break;
        default:
          continue;
      }
